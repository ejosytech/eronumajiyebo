...
class PublicKey extends \WPMailSMTP\Vendor\phpseclib3\Crypt\RSA implements \WPMailSMTP\Vendor\phpseclib3\Crypt\Common\PublicKey
{
     ...
     */
    private function exponentiate(\WPMailSMTP\Vendor\phpseclib3\Math\BigInteger $x)
    {
    ...
    private function rsassa_pkcs1_v1_5_relaxed_verify($m, $s)
    {
        ...
        $decoded = \WPMailSMTP\Vendor\phpseclib3\File\ASN1::decodeBER($em);
        ...
        if (!isset($oids)) {
            ...
            \WPMailSMTP\Vendor\phpseclib3\File\ASN1::loadOIDs($oids);
        }
        $decoded = \WPMailSMTP\Vendor\phpseclib3\File\ASN1::asn1map($decoded[0], \WPMailSMTP\Vendor\phpseclib3\File\ASN1\Maps\DigestInfo::MAP);
    ...
    private function rsaes_pkcs1_v1_5_encrypt($m, $pkcs15_compat = \false)
    {
        ...
        while (\strlen($ps) != $psLen) {
            $temp = \WPMailSMTP\Vendor\phpseclib3\Crypt\Random::string($psLen - \strlen($ps));
    ...
    private function rsaes_oaep_encrypt($m)
    {
        ...
        $db = $lHash . $ps . \chr(1) . $m;
        $seed = \WPMailSMTP\Vendor\phpseclib3\Crypt\Random::string($this->hLen);
    ...
    public function toString($type, array $options = [])
    {
        ...
        if ($type == \WPMailSMTP\Vendor\phpseclib3\Crypt\RSA\Formats\Keys\PSS::class) {
            if ($this->signaturePadding == self::SIGNATURE_PSS) {
                ...
                throw new \WPMailSMTP\Vendor\phpseclib3\Exception\UnsupportedFormatException('The PSS format can only be used when the signature method has been explicitly set to PSS');
            }
        }
    ...
    {
        $new = new \WPMailSMTP\Vendor\phpseclib3\Crypt\RSA\PrivateKey();
