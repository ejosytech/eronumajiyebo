...
class Personalize_Login_Plugin {
	...
	public function __construct() {
		...
		add_shortcode( 'custom-register-form', array( $this, 'render_register_form' ) );
                add_shortcode( 'custom-account-form', array( $this, 'render_account_form' ) );
                add_shortcode( 'custom-password-lost-form', array( $this, 'render_password_lost_form' ) );
                
                add_shortcode( 'custom-password-reset-form', array( $this, 'render_password_reset_form' ) );
                
                // Verification form optional for Application requiring Verify Candidates befor e registering Them
                add_shortcode('Verify-Student-Status', array($this, 'render_verify_status_form'));
	...
	public static function plugin_activated() {
		...
		$page_definitions = array(
			'member-login' => array(
				'title' => __( 'User Flow Sign In', 'personalize-login' ),
				'content' => '[custom-login-form]'
			),
			'member-account' => array(
				'title' => __( 'User Flow Your Account', 'personalize-login' ),
				'content' => '[custom-account-form]'
			),
			'member-register' => array(
				'title' => __( 'User Flow Register', 'personalize-login' ),
				'content' => '[custom-register-form]'
			),
                       'member-password-lost' => array(
                                 'title' => __( 'User Flow Forgot Your Password?', 'personalize-login' ),
                                 'content' => '[custom-password-lost-form]'
                        ),
                        'member-password-reset' => array(
                                'title' => __( 'User Flow Pick a New Password', 'personalize-login' ),
                                'content' => '[custom-password-reset-form]'
                        ),
                        'verify-status' => array(
                                'title' => __('User Flow Verify Student Status', 'personalize-login'),
                                'content' => '[Verify-Student-Status]'
                        )
		);
	...
	public function redirect_to_custom_login() {
		...
		if ( $_SERVER['REQUEST_METHOD'] == 'GET' ) {
			...
			// Initiate the redirected the  login page, In this case the created as 'member-login'
	...
	public function redirect_after_login( $redirect_to, $requested_redirect_to, $user ) {
		...
		if ( user_can( $user, 'manage_options' ) ) {
			...
			//$redirect_url = home_url( 'update-supervisor-visit' );
	...
	public function render_login_form( $attributes, $content = null ) {
        ...
        /**
	 * A shortcode for rendering the user account form.
	 *
	 * @param  array   $attributes  Shortcode attributes.
	 * @param  string  $content     The text content for shortcode. Not used.
	 *
	...
	public function render_account_form( $attributes, $content = null )
            ...
            if ( is_user_logged_in() ) 
                   {
                        return $this->get_template_html( 'account_form', $attributes );
			//return __( 'Display Account Details.', 'personalize-login' );
                   }
         ...
         /**
	 * A shortcode for rendering the user registration form.
	 *
	 * @param  array   $attributes  Shortcode attributes.
	 * @param  string  $content     The text content for shortcode. Not used.
	 *
	...
	public function render_register_form( $attributes, $content = null ) {
        ...
        // Retrieve possible errors from request parameters
         ...
         public function render_verify_status_form($attributes, $content = null) {
        // Parse shortcode attributes
		$default_attributes = array( 'show_title' => false );
		$attributes = shortcode_atts( $default_attributes, $attributes );
                
                return $this->get_template_html('verify_status', $attributes);
                }
	...
	private function register_user( $email, $first_name, $last_name )
       ...
       private function update_user_registration( $email, $matric_no, $last_name, $other_name,$faculty,$dept, $level, $sex,$phone ) 
               {
          // $last_name same as $surname
           
           global $wpdb;
            $var_std_data_table_name = "wpvy_hm_verification";
                try
                {
                    $wpdb->update(
                            $var_std_data_table_name, //$table_name,
                                   
                                array(
                                    surname =>$last_name,// integer (number)
                                    othername =>$othername,
                                    sex=>$sex, 
                                    level=>$level, 
                                    faculty=>$faculty,
                                ...
                                array(verification_no=>$matric_no),
                                array(
                                    '%s', // value1
                                    '%s', // value1
                                    '%s', // value1
	...
	private function get_error_message( $error_code ) {
		switch ( $error_code ) {
			...
			case 'empty_username':
				return __( 'You do have an username, right?', 'personalize-login' );
			...
			case 'invalid_username':
				return __(
					"The username you entered wasn't quite right",
					'personalize-login'
				);
